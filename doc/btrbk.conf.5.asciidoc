...
*compat* <compat-option>::
    Enable a compatibility option. Available 'compat-option':
+
--
*busybox*::
    Use busybox compatible commands.

*missing_otime*::
    Ignore BTRFS_SEND_A_OTIME errors on btrfs receive. Useful for
    synology devices complaining about "ERROR: attribute 12 requested
    but not present".
